...
function WorkSlide({
  ...
  const base =
    horizontal
      ? `editorial-panel relative flex h-[100dvh] w-screen shrink-0 select-none flex-col justify-center overflow-hidden px-6 py-16 md:px-14 ${border}`
  ...
  return (
    <section id={workIds[wi]} className={base}>
      ...
      <div
        className={`deck-fg relative z-10 mx-auto grid w-full max-w-6xl items-center gap-10 md:gap-16 ${
      ...
      >
            ...
            {imageUrl ? (
              <>
                <Image
                  ...
                  height={frameH}
                  draggable={false}
                  className="h-auto max-h-[min(84dvh,1024px)] w-auto max-w-full object-contain"
                  sizes="(max-width: 768px) 96vw, min(48vw, 820px)"
                  priority={wi === 0}
                />
                <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/35 via-transparent to-black/12" />
              </>
            ) : (
              <div className="relative min-h-[min(50dvh,420px)] w-full min-w-[min(100%,20rem)]">
        ...
        >
          <div
            ...
            data-lenis-prevent
            className={`max-h-[min(52dvh,28rem)] w-full max-w-md select-text overflow-y-auto overscroll-y-contain pr-1 [scrollbar-color:rgba(255,255,255,0.25)_transparent] [scrollbar-width:thin] md:max-h-[min(58dvh,32rem)] ${
              flip ? "md:ml-auto" : ""
            }`}
          >
...
export function WorkHorizontalGallery() {
  ...
  useGSAP(
    () => {
      ...
      /** Scrub + snap: soft follow without overshooting between slides (pairs with Lenis). */
    ...
    <section id="work">
      <div className="mx-auto max-w-6xl px-6 pt-16 md:px-14 md:pt-20">
        ...
        <p className="mt-3 max-w-xl text-sm leading-relaxed text-[var(--muted)]">
          Scroll smoothly through the page to move the gallery—drag sideways on desktop or
          use a horizontal trackpad gesture. Long captions scroll inside the text column.
        </p>
      </div>
